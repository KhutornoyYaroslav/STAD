...
def main() -> int:
    ...
    parser.add_argument('-o', '--output-dir', dest='output_dir', required=False, type=str,
                        default="/media/yaroslav/SSD/khutornoy/data/sim_videos/results",
    ...
    for video in sorted(glob(args.input)):
        ...
        while reader.isOpened():
            ...
            with torch.no_grad():
                ...
                outputs_person = non_max_suppression(out_y, conf_thres=args.conf, iou_thres=0.45, classes=[0, 1, 2, 3, 4], agnostic=True, in_place=False)[0]
            ...
            for det in detects:
                ...
                cv.rectangle(frame, (x1, y1), (x2, y2), color, 2) # NMS
